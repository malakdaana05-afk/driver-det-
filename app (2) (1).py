...
# =========================
# PREPROCESS — نفس التدريب بالظبط
# =========================
def preprocess_image(image, cls=None):
    img = np.array(image.convert('RGB'))

    # CLAHE على نفس الكلاسيس اللي تدرّبنا عليها
    ...
    # Letterbox resize — نفس ImageOps.pad
    img = np.array(ImageOps.pad(Image.fromarray(img), IMG_SIZE, color=(0, 0, 0)))
...
if uploaded_file is not None:
    try:
        ...
        with st.spinner("Analyzing Driver State..."):
            # جرّب كل الكلاسيس وخد أعلى confidence
            all_preds = []
            for cls in CLASS_NAMES:
                processed = preprocess_image(image, cls=cls)
                pred = model.predict(processed, verbose=0)[0]
                all_preds.append(pred)

            # خد متوسط أو الـ prediction بدون CLAHE (الأكثر عمومية)
